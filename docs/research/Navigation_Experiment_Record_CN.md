...
固定 **导航相关** 实验事实（`batch_id`、模版路径、关键表），避免结果散在终端。**不**承担完整论文叙事：判停细则、MI 口径、第二阶段总叙事以 **`Major_Issues_And_Resolutions_CN.md`**、**`SSGS_Research_Framework_CN.md`** 为准；本文件 **§9 起** 多为 **历史表归档**；**当前默认旋钮与近期单变量结论** 优先看 **§6.0、§6.5～6.7**。
