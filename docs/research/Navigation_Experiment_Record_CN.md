...
**读法**：相对 **`probe2` rule 满 500（`041200Z`，`never_visit≈0.58`）**，**`α=0.05` 冻结行 `081727Z`** 将 **`never_visit` 压到约 0.49** 量级；**`α=0.1` 满 500（`095212Z`）** 达 **阶段 B**；**`α=0.15` 满 500（`105756Z`）** 再将 **`never_visit` 压至 0.42**、**`gold_visit_dedup` 抬至 0.58**，**阶段 A（收工级）已达成**。**下一刀（可选，仍单变量）**：**`max_nodes=96`** 作天花板压力（模版：`configs/experiment/navigation_batch_real_corpus_p0_visit_rule_entity_boost_a015_abl_maxnodes_96.example.json`）；或 **e2e（过程 first）**。**`learned_root` / e2e** 与检索 EM **分列**；e2e 仅在 **过程仍 ≤0.12** 且你愿意付 7B 成本时开。
...
1. **顺序**：先 **`entity_boost_alpha`**（**`0.05`→`0.1`→`0.15`**，路由偏置；**`probe_top_m=1`** 下不稀释 **`probe_budget`**，cap 失控风险相对低）；再 **`max_nodes`**（**`80`→`96`**，硬扩图，同步盯 **`avg_nav_wall_time_ms` / 尾延迟**）。**`max_nodes`** 仅作 **α 网格后的天花板压力测试**；若 **α 连续两格已熔断**，**收口**、**不**扫第三格。**锚点**：**`α=0.05`** → **`081727Z`**；**`α=0.1` 满 500** → **`095212Z`**（**B 档**）；**`α=0.15` 满 500** → **`105756Z`**（**A 档**）。  
...
**下一步（执行顺序，2026-04-18 修订）** — **P0-A′（`probe_budget`）已闭合**；**P0-B′：`α=0.05` 冻结 + `α=0.1`（`095212Z`）达 B + `α=0.15`（`105756Z`）达 A**（上表）。工程待办与 e2e 重跑句仍见下 **bash** 块；**导航主线（可选）**：**`max_nodes=96`** 天花板压力 或 **e2e**；**`probe_top_m=2`** 已证 trade-off，**不**与当前默认并冻。
...
**导航侧下一阶段（由前到后）**：**P0-A′ 导航**（**`probe1`/`probe2` 满 500**）已闭合。**P0-B′**：**`α=0.05`**（**`081727Z`**）冻结；**`α=0.1`（`095212Z`）** 达 **B**；**`α=0.15` 满 500（`105756Z`）** 达 **A**（**`never_visit≈0.42`**、**`gold_visit_dedup≈0.58`**）。余量主矛盾在 **预算饱和（`gold_miss_evi`）** 与 **延迟成本**，下一格（可选）：**`max_nodes=96`** 或 **e2e**；**非**再扫 `α` / `context_select`。**端到端 `probe2`** 与 **`learned_root`** 另线；**勿**用 `leaf_indices_required` 作弊。
