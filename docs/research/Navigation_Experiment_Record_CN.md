...
**P0-B′ 导航扫参收口（`2026-04-18`）** — **默认工作点**：**`122155Z` 同旋钮**（**`entity_boost_alpha=0.3`**，**`max_nodes=80`**，其余与 **`081727Z`** 冻结模版一致：`configs/experiment/navigation_batch_real_corpus_p0_visit_rule_entity_boost_a030.example.json`）。**保守对照**：**`105756Z`（`α=0.15`）**。**本轮扫参不再默认续开**：**`α` 细网格**（**`0.2` 烟测边际**已记）、**`context_select_*`**、**`probe_top_m=2`**（trade-off 已记）、**`max_nodes=96`**（**`a030_abl_maxnodes_96`** 烟测 **`125340Z`**，**`n=200` 汇总与 `α=0.3@80` 烟测 `114138Z` 同表**，**不优先**为默认再抬节点预算；若日后需证伪再跑 **单次满 500** 即可）。**仍存优化空间（承认有余量，但不并入本条扫参闭环）**：**`gold_miss_evi` 仍高**（**`max_evidence=12` 饱和**）→ **`max_evidence` / accept 排序与去重 / cap-minrel 联合** 须 **另立项** 并与 **accept 冻结策略**对齐；**探索拓扑**（回溯、子扇出、**非**靠再抬 **`α`**）；**路径深度/浅支聚集**（需 **payload 级深度直方图** 才有把握归因）；**`learned_root` 与深训 Router**（见 **§7**、与导航扫参 **分列**）；**墙时与尾延迟**（`avg_nav_wall_time_ms`、端到端成本）。**下一主工作量**：**e2e** 用 **`122155Z` 旋钮** 复验 **过程 + 终点 EM 同向**，再决定是否进入 **learned / Router 深调**。（**P0 `end_to_end_*`** 已写 **`entity_boost_alpha=0.3`** 及 **`question_entity_match_topk` / `max_evidence=12` / `min_rel=0.6` / `max_nodes=80`**；**`batch_id_prefix` / `run_id_prefix`** 追加 **`_visit_a030`**，与旧 **`overlap_topk` 跑批** 区分。）

**①b（`2026-04-18`）**：在 **`122155Z` 旋钮** 上 **仅** **`explore_root_probe_top_m: 1→2`** 做 **N=200 烟测**（模版：`configs/experiment/navigation_batch_real_corpus_p0_visit_rule_entity_boost_a030_abl_probe_top_m2.example.json`），看 **`never_visit` 是否下降**；接受 **`avg_nav_wall_time_ms` 上升**。**与历史 **`probe_top_m=2` 满 500（`071832Z`，模版 `navigation_batch_real_corpus_p0_visit_rule_root_probe_top_m2.example.json`）** 不同**：旧臂为 **`overlap_topk` / `max_evidence=8` / `min_rel=1.0` / `max_nodes=64` / 无 `entity_boost`**，结论为 **trade-off**（见 **`Major_Issues_And_Resolutions_CN.md`** 表行 **P0-B-1**）。**复验（`α=0.3`+visit-rule 栈）**：**`142055Z`，`n=200`** — 与 **`114138Z`** 同口径相比 **`never_visit` 仍为 0.40（不降）**、**`visit_miss_accept` 0.13（>0.12）**、**`gold_in_accept` 略降** → **不推进满 500**，**默认仍 `probe_top_m=1`**。

**①a（`2026-04-18`，工程挂钩）**：针对 **`never_visit` 桶内「根层子节点 `entity_match_score` 全 0」** 的常见模式，在 **`depth==0`**、**`entity_boost_alpha>0`**、**题面已抽出实体** 且 **所有根子节点 `entity_match_score` 均为 0** 时，对子节点分数追加 **`root_entity_zero_overlap_fallback_beta * keyword_token_overlap_fraction(question, child.text)`** 并重排（**`beta=0` 为默认，行为与旧版一致**）。**烟测协议**：与 **`114138Z` / `122155Z`** 同 manifest 前 **`N=200`**，盯 **`never_visit`**、**`visit_miss_accept`（须 ≤0.12）**、**`avg_nav_wall_time_ms`**；单变量模版：`configs/experiment/navigation_batch_real_corpus_p0_visit_rule_entity_boost_a030_abl_root_overlap_fallback_b007.example.json`（示例 **`beta=0.07`**，可按需改键值另存前缀）。**`route_decisions.child_scores`** 可选带出 **`question_keyword_overlap`**；**`trace.event_log`** 在触发时追加 **`root_entity_zero_overlap_fallback`**（含 **`beta`**）。**复跑（`β=0.07`，`n=200`）**：**`144709Z`**（`nav_p0_visit_rule_entity_boost_a030_abl_root_overlap_fb_b007_20260418_144709Z`）— **`never_visit` 桶占比 0.395**（与 **`114138Z`≈0.40** 同量级，**未显降**）、**`visit_miss` 0.10（≤0.12，优于①b 的 0.13）**；`visit_miss` 叶级 disposition **`reject_leaf_branch_cap=25` / `reject_leaf_min_relevance=4`**；审计全量 **`outputs/reports/accept_gate_audit_…144709Z.json`**，分桶 CSV **`audit_sample_buckets_…144709Z.csv`**。**结论（烟测）**：**过程门可过**，但 **`never_visit` 未打开新收益** → **不列为必跑满 500 的强推进项**；默认 **`beta=0` 保持与 `122155Z` 栈一致**；若论文侧要关门可 **单次满 500 证伪**，算力优先仍建议 **e2e / evidence-accept 另项**。

**② `never_visit` 优先（结构单变量，`2026-04-18` 续）**：在 **`122155Z` 同旋钮** 上只做 **一项** 放松 **探索硬顶**，与 **`114138Z`（`n=200`）** 对照 **`never_visit` 桶占比**；并行硬门 **`visit_miss`（≤0.12）**、**`avg_nav_wall_time_ms`**。**已有机位**：**`max_nodes=96`**（`navigation_batch_real_corpus_p0_visit_rule_entity_boost_a030_abl_maxnodes_96.example.json`，历史 **`125340Z`** 烟测与 `114138Z` 同表不显性）。**本轮新增烟测模版**（仍 **`n=200`**）：**`max_nodes: 80→112`** → `navigation_batch_real_corpus_p0_visit_rule_entity_boost_a030_abl_maxnodes_112.example.json`；**`max_depth: 8→10`（`max_nodes` 仍为 80）** → `navigation_batch_real_corpus_p0_visit_rule_entity_boost_a030_abl_maxdepth_10.example.json`。**推进规则**：若 **`never_visit` 较 `114138Z` 降 ≥2pp** 且 **`visit_miss` 不顶穿 0.12**，可记 **阳性候选** 再决定是否 **单次满 500**；否则 **台账记阴性**、仍默认 **`122155Z`（80/8）**。**复测 `max_nodes=112`（`n=200`）**：**`151646Z`**（`nav_p0_visit_rule_entity_boost_a030_abl_maxnodes_112_20260418_151646Z`）— **`never_visit` 桶占比 0.40**（与 **`114138Z`≈0.40** 同档，**未达 ≥2pp**）、**`visit_miss` 0.10（≤0.12）**；`visit_miss` 叶级 **`reject_leaf_branch_cap=25` / `reject_leaf_min_relevance=4`**。**结论**：**不推进满 500**，**默认仍 `max_nodes=80`**。**复测 `max_depth=10`（`n=200`）**：**`153821Z`**（`nav_p0_visit_rule_entity_boost_a030_abl_maxdepth_10_20260418_153821Z`）— **`never_visit` 0.40**、**`visit_miss` 0.10**、叶级 **cap 25 / minrel 4**，与 **`151646Z` / `114138Z`** 同档 → **② 结案**：**`max_nodes`/`max_depth` 单变量放松** 在 **`n=200`** 上 **未带来 `never_visit` 收益**；**默认仍 `max_nodes=80`、`max_depth=8`**。**归因（`151646Z` 审计 + `summarize_never_visit_nav_signals.py`）**：**`never_visit` 行 80/80** 可解析 payload；**`max_nodes_reached` 占比 0.0**、**`max_depth_reached` 占比 0.0**；**`root_probe_plan` 占比 1.0**；**`mean(len(visited_node_ids))≈30`**、**`mean(len(evidence_texts))=12.0`（打满 `max_evidence`）**、**`mean_rollback≈1.95`**。**读法**：**`never_visit` 主因不是「探到 `max_nodes`/`max_depth` 顶」**；更似 **根排序 + DFS/回溯在证据槽打满前未走入含金子枝**（与继续盲抬 **`max_nodes`/`max_depth`** 的低 ROI 一致）。**下一主方向**：**(A)** **`visit_miss`/accept**（**`branch_cap`/minrel/配额**）；**(B)** **探索拓扑/回溯**（**非**再抬 **`max_nodes`/`max_depth`** 细网格）；可选对 **`153821Z`** 审计再跑同一脚本 **对表**。**工具**：`python scripts/diagnostics/summarize_never_visit_nav_signals.py --audit-json outputs/reports/accept_gate_audit_<batch>.json --root .`
...
**满 500 导航（`probe_budget=3`）**：**`162118Z`**（`nav_p0_visit_rule_entity_boost_a030_nav500_probe_budget3_20260418_162118Z`）— **`frac_samples_never_visit_any_gold=0.384`**（较 **`122155Z`** **0.378** 约 **+0.6pp**，**未达 ≥2pp** 熔断）、**`frac_samples_visit_gold_but_missing_accept…=0.066`**（较 **`122155Z`** **~0.110** 约 **−4.4pp**）；`visited_not_accepted_dispositions_aggregated`：**`reject_leaf_branch_cap=42` / `minrel=10`**（叶次）；**`frac_samples_with_any_gold_in_context=0.616`**，**`mean_frac_gold_leaves_in_context≈0.256`**，**`sum_accepted_gold_not_in_context=70`**。审计 JSON **`accept_gate_audit_…162118Z.json`**，分桶 CSV **`audit_sample_buckets_…162118Z.csv`**。**结论**：**满量 trade-off 与 `n=200`（`160226Z`）同向**——**`visit_miss` 显著改善**，**`never_visit` 略升但在熔断线内** → **不替换 `122155Z` 默认（`probe_budget=2`）**；若业务 **强偏 accept/context 金叶** 可将 **`probe_budget=3`** 记为 **并列 B′ 臂**，**须**与 **`122155Z` 同 manifest e2e 的 `exact_match_rate`** 对表后再写主叙事。

**④ 导航侧逼近 Oracle 上界（不做 e2e、非作弊，`2026-04-18`）**：**`oracle_item_leaves` / 注入 `leaf_indices_required` 仍为作弊上界**（仅作 gap 参照，见 **§9** 与 **`Navigation_Experiment_Record_CN.md`** 较早 **「勿混三件事」**）。本段指 **在真实导航下** 抬 **`frac_gold_leaf_ever_visited` / 压 `never_visit` / 抬 `frac_gold_in_accepted_evidence` / `mean_frac_gold_leaves_in_context`**，**仅跑导航批 + `audit_accept_gate`**，**本轮不写 e2e 必做项**。**锚**：**`122155Z`（`rule`、`probe_budget=2`）**；**硬门**：**`visit_miss`（≤0.12）**；**烟测优先 `n=200`**，阳性再 **满 500**。**单变量模版（各仅一项相对 `122155Z`）**：**(i)** **混合根学习头 `α=0.5`**（checkpoint **`configs/router/learned_root_router_real_corpus.json`**）→ `navigation_batch_real_corpus_p0_visit_rule_entity_boost_a030_learned_root_blend05.example.json`；**(ii)** **`routing_mode: cosine_probe`**（其余同 **`122155Z`**）→ `navigation_batch_real_corpus_p0_visit_rule_entity_boost_a030_cosine_probe.example.json`；**(iii)** **`max_evidence: 12→14`**（缓解 **证据槽早满** 与探索早停耦合，**`151646Z` `never_visit` 归因已见 `mean(evidence_texts)=12` 打满**）→ `navigation_batch_real_corpus_p0_visit_rule_entity_boost_a030_abl_maxev_14.example.json`。**判读**：与 **`122155Z`/`162118Z`** 对 **`never_visit`、`visit_miss`、`mean_frac_gold_leaves_in_context`、`sum_accepted_gold_not_in_context`**；**`learned`/`cosine`** 与 **`rule` 主默认** **分列**（叙事见 **§6.5**、**MI-004/005**）。
...
**导航侧下一阶段（由前到后）**：**P0-A′ 导航**（**`probe1`/`probe2` 满 500**）已闭合。**P0-B′ 扫参收口**：**`rule` 实体偏置** 默认 **`122155Z`**（**`α=0.3`、`max_nodes=80`**；**`105756Z`** 保守对照）。**承认**：**`gold_miss_evi`、墙时、浅支路径** 等仍有优化余量（见 **「P0-B′ 导航扫参收口」** 段 **「仍存优化空间」**），**不**再默认续扫 **`α`/`context_select`/`max_nodes`** 细网格。**优先**：**e2e**（同旋钮、过程 first）；**证伪/深挖** 另开子项。**端到端 `probe2`** 与 **`learned_root`** 另线；**勿**用 `leaf_indices_required` 作弊。
