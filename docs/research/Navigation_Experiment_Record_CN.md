...
| ④′ | **`cosine_probe`**（**`a030` 实体偏置栈**，与 **`…a030_cosine_probe.example.json`** 一致） | **500** | **`nav_p0_visit_rule_entity_boost_a030_cosine_probe_20260419_081150Z`**（以服务器 **`run_registry.jsonl`** / **`batch_summary.json`** 为准） | **`frac_samples_never_visit_any_gold=0.214`** | **`frac_samples_visit_gold_but_missing_accept…=0.19`** | **相对 `122155Z`（`never_visit≈0.378`、`visit_miss≈0.11`）**：**到金叶**明显更好，**visit 后 accept 缺口**明显更大；**ctx-gold**：**`frac_samples_with_any_gold_in_context=0.784`**，**`mean_frac_gold_leaves_in_context≈0.360`**，**`sum_accepted_gold_not_in_context=84`**；叶次 **`reject_leaf_branch_cap=116` / `reject_leaf_min_relevance=18`** → 瓶颈更像 **分支预算 + minrel**，而非「从未 visit」。**下一步**：同 manifest 对 **`batch_summary` 检索 EM**（及视叙事需要 **e2e**），**勿**与 §9.12 旧 **`context_select` 三连**混表。**审计落盘**：若 **`audit_accept_gate.py --out-json`** 写成**不带 `.json` 后缀**的路径，则磁盘上为**无扩展名文件**，查找 **`…081150Z.json`** 会 **Not found**——应 **`ls …081150Z`** 或 **`mv` 为 `.json`**；推荐以后固定 **`--out-json "outputs/reports/accept_gate_audit_${BATCH_ID}.json"`**。 |
