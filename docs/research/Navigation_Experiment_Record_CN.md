...
**② `never_visit` 优先（结构单变量，`2026-04-18` 续）**：在 **`122155Z` 同旋钮** 上只做 **一项** 放松 **探索硬顶**，与 **`114138Z`（`n=200`）** 对照 **`never_visit` 桶占比**；并行硬门 **`visit_miss`（≤0.12）**、**`avg_nav_wall_time_ms`**。**已有机位**：**`max_nodes=96`**（`navigation_batch_real_corpus_p0_visit_rule_entity_boost_a030_abl_maxnodes_96.example.json`，历史 **`125340Z`** 烟测与 `114138Z` 同表不显性）。**本轮新增烟测模版**（仍 **`n=200`**）：**`max_nodes: 80→112`** → `navigation_batch_real_corpus_p0_visit_rule_entity_boost_a030_abl_maxnodes_112.example.json`；**`max_depth: 8→10`（`max_nodes` 仍为 80）** → `navigation_batch_real_corpus_p0_visit_rule_entity_boost_a030_abl_maxdepth_10.example.json`。**推进规则**：若 **`never_visit` 较 `114138Z` 降 ≥2pp** 且 **`visit_miss` 不顶穿 0.12**，可记 **阳性候选** 再决定是否 **单次满 500**；否则 **台账记阴性**、仍默认 **`122155Z`（80/8）**。**复测 `max_nodes=112`（`n=200`）**：**`151646Z`**（`nav_p0_visit_rule_entity_boost_a030_abl_maxnodes_112_20260418_151646Z`）— **`never_visit` 桶占比 0.40**（与 **`114138Z`≈0.40** 同档，**未达 ≥2pp**）、**`visit_miss` 0.10（≤0.12）**；`visit_miss` 叶级 **`reject_leaf_branch_cap=25` / `reject_leaf_min_relevance=4`**。**结论**：**不推进满 500**，**默认仍 `max_nodes=80`**。**下一笔**：同协议跑 **`abl_maxdepth_10`**（`navigation_batch_real_corpus_p0_visit_rule_entity_boost_a030_abl_maxdepth_10.example.json`，**`n=200`** + 同上 **`audit_accept_gate` / `summarize_audit_failure_buckets`**），为 **②** 结案。**归因（只读、不改代码）**：对任一 **`accept_gate_audit_<batch>.json`** 运行  
`python scripts/diagnostics/summarize_never_visit_nav_signals.py --audit-json outputs/reports/accept_gate_audit_<batch>.json --root .`  
看 **`never_visit` 行**里出现 **`max_nodes_reached` / `max_depth_reached` 的占比**：**占比低** → **`never_visit` 多半不是「顶到硬顶」而是排序/子树选择**；**占比高** → 再考虑抬 **`max_nodes`/`max_depth`** 或放松 **证据早满** 导致的提前返回（与 **`max_evidence`** 链分列分析）。
