...
1. **顺序**：先 **`entity_boost_alpha`**（**`0.05`→`0.1`→`0.15`→`0.3`** 等，路由偏置；**`probe_top_m=1`** 下不稀释 **`probe_budget`**，cap 失控风险相对低）；再 **`max_nodes`**（**`80`→`96`**，硬扩图，同步盯 **`avg_nav_wall_time_ms` / 尾延迟**）。**`max_nodes`** 仅作 **α 网格后的天花板压力测试**；若 **α 连续两格已熔断**，**收口**、**不**扫第三格。**锚点**：**`α=0.05`** → **`081727Z`**；**`α=0.1` 满 500** → **`095212Z`**（**B 档**）；**`α=0.15` 满 500** → **`105756Z`**（**A 档 / 保守**）；**`α=0.3` 满 500** → **`122155Z`**（**当前默认候选**）。  
...
5. **主表与后置**：新 e2e 默认跑完 **覆盖 P0 主表 A 行 `batch_id`**，旧 **`154358Z`** 可作 **A′（历史 `probe1`）**。**金叶可达性**已由 **`122155Z`（`never_visit≈0.38`）** 等批跨过旧 **`<45%` 叙事阈值**；**Router / learned 深调** 仍建议 **在 e2e 闭环评估之后** 再开，避免与 **实体偏置扫参主线** 并行失焦；**accept 盲扫不解冻**。

**下一步（执行顺序，2026-04-18 修订）** — **P0-A′（`probe_budget`）已闭合**；**P0-B′：`α=0.05` 冻结 + `α=0.1`（`095212Z`）达 B + `α=0.15`（`105756Z`）达 A + `α=0.3`（`122155Z`）为当前默认候选**（上表）。工程待办与 e2e 重跑句仍见下 **bash** 块；**导航主线（可选）**：**`max_nodes=96`（`α=0.3`，`a030_abl_maxnodes_96`）** 天花板压力 或 **e2e**；**`probe_top_m=2`** 已证 trade-off，**不**与当前默认并冻。
...
**导航侧下一阶段（由前到后）**：**P0-A′ 导航**（**`probe1`/`probe2` 满 500**）已闭合。**P0-B′**：**`α=0.05`**（**`081727Z`**）冻结；**`α=0.1`（`095212Z`）** 达 **B**；**`α=0.15`（`105756Z`）** 达 **A**；**`α=0.3` 满 500（`122155Z`）** 为 **`rule` 实体偏置臂当前默认候选**（**`never_visit≈0.38`**、**`gold_visit_dedup≈0.62`**，`visit…missing_accept` 仍 **≤0.12**）。余量主矛盾在 **预算饱和（`gold_miss_evi`）** 与 **延迟成本**，下一格（可选）：**`max_nodes=96`（保持 `α=0.3`）** 或 **e2e**；**非**再扫 **`context_select`**。**端到端 `probe2`** 与 **`learned_root`** 另线；**勿**用 `leaf_indices_required` 作弊。
