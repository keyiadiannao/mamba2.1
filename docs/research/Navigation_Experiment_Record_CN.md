...
**①b（`2026-04-18`）**：在 **`122155Z` 旋钮** 上 **仅** **`explore_root_probe_top_m: 1→2`** 做 **N=200 烟测**（模版：`configs/experiment/navigation_batch_real_corpus_p0_visit_rule_entity_boost_a030_abl_probe_top_m2.example.json`），看 **`never_visit` 是否下降**；接受 **`avg_nav_wall_time_ms` 上升**。与历史 **`probe_top_m=2` trade-off** 对照见 §6.5/扫参约束。
