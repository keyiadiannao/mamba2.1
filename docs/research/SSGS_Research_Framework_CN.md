...
**叙事收窄（批判性共识）**：层次化/树状 RAG、图上或树上的多步搜索与回溯、以及 Mamba 在检索管线中作编码器或重排器，**均有大量先行工作**。本课题若将创新简单写成「树 + 回溯 + Mamba」的并列组合，**辨识度不足**；对外与对审稿更稳妥的表述是：**在受控树上做多步回溯时，以 Mamba 固定维状态快照作为路径状态载体的价值与边界**——回溯与搜索协议是**验证该载体的手段**，树是**承载问题的结构**。下文 **§4** 的研究问题已按此收窄重述。
...
1. **核心科学关切**在于：**Mamba 固定维隐状态与节点级快照恢复**，能否作为树上多步导航中**低开销、可恢复的路径状态载体**；树状 RAG 与显式回溯协议主要用于**提供可检验场景与评测接口**。
...
- **Navigator 实现口径（与「Mamba 载体」主张对齐）**：在 **`Mamba2Navigator` + `hf_pretrained`** 路径下，每步对**当前节点文本**做一次模型前向；跨步的「路径摘要」主要来自 **`merge_path_summaries`（向量级融合）** 与 **`hidden_summary` 快照**，**不等价于**将「整条路径的原始文本」作为**单次**长序列送入 SSM 的递推扫描。论文叙事须与此一致。仓库提供 **`navigator_type=sentence_transformer`**（默认 **`sentence-transformers/all-MiniLM-L6-v2`**，384 维）在**同一融合与同一控制器**下作 **Mamba vs 句向量编码器** 的诊断对照（见 **`Navigation_Experiment_Record_CN.md`**「句向量诊断」）。
