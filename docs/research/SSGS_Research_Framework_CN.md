...
在真实树状 RAG 场景中，构建一个以 Mamba 为导航器、以 Transformer 为生成器、支持状态快照回溯与完整 trace 审计的最小闭环系统，为后续比较实验、结构化图应用扩展和论文撰写建立统一框架。
...
做出一个真实可运行的、树状的、可回溯的、可审计的、导航与生成解耦的 RAG 框架，并为后续结构化图场景保留自然扩展接口。
