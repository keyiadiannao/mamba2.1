...
相对「以搜索树/历史栈/重算 KV 前缀等**其他方式**承载回溯所需信息」，**固定维快照 + O(1) 级恢复** 在 **延迟、内存占用、证据与过程指标** 上是否呈现**可复现的 trade-off**？此处强调**对照轴是「回溯状态载体的实现形态」**，而非与「常见 RAG 是否分成检索器与 LLM」混为一谈。

### RQ3：适用条件与外部效力

在 **树深度、扇出、分支因子** 等结构特征变化时，快照恢复带来的**边际收益**如何分布（例如在**深树、多回溯**场景是否更明显）？**论文阶段**需补充与**同类层次导航或树搜索式 RAG** 的**有限外部基线**（规模与算力预算预先声明），否则结论易停留在**系统内部 Router 消融**，外部效力不足——本条承认**当前工程阶段尚未完成该类对比**，作为后续硬项写入计划。
...
- **控制器学习化**：当前 **SSGS** 以**可解释、可复现**的规则控制为主；与强学习化树策略的对比可作为**后续变体或局限**，不必假装已在主线中全面覆盖。
